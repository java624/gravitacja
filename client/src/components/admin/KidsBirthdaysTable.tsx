...
import type { BirthdayInquiry, BirthdayInquiryStatus } from '../../types/birthday';
import { fetchBirthdayInquiries, updateBirthdayInquiryStatus, BIRTHDAY_INQUIRIES_EMAIL } from '../../lib/supabase';

const STATUS_LABELS: Record<BirthdayInquiryStatus, string> = {
  new: 'Nowe',
...
const STATUS_COLORS: Record<BirthdayInquiryStatus, string> = {
...
export default function KidsBirthdaysTable() {
  const [inquiries, setInquiries] = useState<BirthdayInquiry[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [filter, setFilter] = useState<'all' | BirthdayInquiryStatus>('all');

  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const data = await fetchBirthdayInquiries();
        if (!cancelled) setInquiries(data);
      } catch (err) {
        console.error('Error loading birthday inquiries:', err);
      } finally {
  ...
  const handleStatus = async (id: string, status: BirthdayInquiryStatus) => {
    try {
      await updateBirthdayInquiryStatus(id, status);
    ...
    <div className="bg-slate-950/80 border border-white/15 rounded-3xl backdrop-blur-2xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.8)]">
      <div className="p-4 border-b border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1 p-1 rounded-xl bg-white/5 border border-white/10">
            {(['all', 'new', 'contacted', 'confirmed', 'closed'] as const).map((s) => (
              <button
                key={s}
                onClick={() => setFilter(s)}
      ...
      {isLoading ? (
        ...
        <div className="p-10 text-center space-y-2">
          ...
          <p className="text-xs text-slate-400 font-medium">Brak zapytań o urodziny.</p>
          <p className="text-[10px] text-slate-600">Zgłoszenia z formularza na stronie /katowice/dzieci pojawią się tutaj.</p>
        </div>
      ) : (
          ...
          {filtered.map((q) => (
            <div key={q.id} className="p-4 hover:bg-white/[0.03] transition-colors grid grid-cols-1 lg:grid-cols-12 gap-3">
              ...
              <div className="lg:col-span-3">
                <div className="text-xs font-black text-white uppercase">{q.parent_name}</div>
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-slate-300 mt-2">
                  ...
                  <span className="font-bold text-white">{q.child_name || '—'}</span>
                  {q.child_age && <span className="text-slate-500">({q.child_age} lat)</span>}
                </div>
              </div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {q.event_time && (
                    <>
                      ...
                      <span>{q.event_time}</span>
                    </>
                  )}
                </div>
                <div className="flex items-center gap-1.5">
                  <Users className="w-3 h-3 text-purple-400" />
                  <span>{q.guests_count ? `${q.guests_count} dzieci` : 'Liczba dzieci: —'}</span>
                </div>
                {q.package_type && (
                  <div className="flex items-center gap-1.5">
                    <Package className="w-3 h-3 text-amber-400" />
                    <span className="font-bold text-white uppercase">{PACKAGE_LABELS[q.package_type] || q.package_type}</span>
                  </div>
                )}
                {q.extras && q.extras.length > 0 && (
                  <div className="flex items-start gap-1.5">
                    <Gift className="w-3 h-3 text-rose-400 mt-0.5" />
                    <span className="flex flex-wrap gap-1">
                      {q.extras.map((e) => (
                        <span key={e} className="px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-[9px] uppercase font-black text-slate-300">
                          {e.replace(/-/g, ' ')}
                        </span>
                      ))}
                    </span>
              ...
              <div className="lg:col-span-4">
                {q.notes ? (
                  <div className="flex items-start gap-1.5">
                    ...
                    <p className="text-[11px] text-slate-400 leading-relaxed line-clamp-3">{q.notes}</p>
                  </div>
                ) : (
              ...
              <div className="lg:col-span-2 flex items-start lg:justify-end gap-2">
                <select
                  value={q.status}
                  onChange={(e) => handleStatus(q.id, e.target.value as BirthdayInquiryStatus)}
      ...
      <div className="p-3 px-4 border-t border-white/10 text-[10px] text-slate-500 flex flex-col sm:flex-row justify-between gap-1">
        <span>E-mail zespołu: <span className="font-mono">{BIRTHDAY_INQUIRIES_EMAIL}</span></span>
