...
export default function Header({ selectedCity, onSelectCity }: HeaderProps) {
  ...
  return (
    <header className={`sticky top-0 z-50 w-full transition-all duration-500 ${isScrolled ? 'py-2.5 sm:py-3.5' : 'py-4 sm:py-6'}`}>
      <div className="max-w-7xl mx-auto px-3 sm:px-6 relative">
        ...
        {/* Головний контейнер Хедера з ефектом преміального скла */}
        <div className={`relative rounded-[22px] sm:rounded-[28px] border transition-all duration-500 flex justify-between items-center overflow-hidden ${
          isScrolled
            ? 'p-2.5 md:px-7 md:py-3 border-white/15 bg-slate-950/85 backdrop-blur-2xl shadow-[0_15px_40px_rgba(0,0,0,0.9),0_0_25px_rgba(249,115,22,0.15)]'
            : 'p-3 md:px-8 md:py-4 border-white/10 bg-slate-950/75 via-slate-900/60 to-black/80 backdrop-blur-xl md:backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.8)]'
        }`}>
          ...
          <motion.nav
            ...
            transition={{ duration: 0.6 }}
            className="hidden md:flex items-center gap-1.5 bg-black/50 p-1.5 rounded-2xl border border-white/10 backdrop-blur-xl relative z-10 shadow-inner"
          >
            <button
          ...
          <button
            ...
            className="md:hidden relative z-10 p-2.5 sm:p-3 rounded-2xl bg-white/5 border border-white/10 text-white hover:bg-orange-500/20 hover:border-orange-500/40 transition-all active:scale-95"
            aria-label="Меню"
          >
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div
              ...
              className="md:hidden overflow-hidden mt-2.5"
            >
              <div className="rounded-[22px] border border-white/15 bg-slate-950/95 backdrop-blur-2xl p-4 flex flex-col gap-2.5 shadow-[0_20px_50px_rgba(0,0,0,0.95)]">
