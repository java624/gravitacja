...
export default function Header({
  ...
  const { activeSlug, activeLocation, setActiveSlug, openBooking } = useLocationContext();

  const [mobileOpen, setMobileOpen] = useState(false);
  const [isScrolled, setIsScrolled] = useState(false);
  const [cityDropdownOpen, setCityDropdownOpen] = useState(false);

  const currentSlug: LocationSlug = activeSlug || 'katowice';
  const currentLocation = activeLocation || LOCATIONS_DATA[1];
  ...
  const handleSelectCity = (newSlug: LocationSlug) => {
    ...
    // Dynamic URL subpath preservation
  ...
  const navItems = [
    { label: 'O Lokalu', path: `/${currentSlug}`, icon: MapPin },
    { label: 'Cennik', path: `/${currentSlug}/cennik`, icon: DollarSign },
    { label: 'Rezerwacje', path: `/${currentSlug}/rezerwacje`, icon: Calendar },
    { label: 'Dzieci', path: `/${currentSlug}/dzieci`, icon: Cake },
    { label: 'Firmy', path: `/${currentSlug}/firmy`, icon: Briefcase },
    { label: 'Menu', path: `/${currentSlug}/menu`, icon: Utensils },
    { label: 'Klub', path: `/${currentSlug}/klub`, icon: Flame },
    { label: 'Praca', path: `/${currentSlug}/praca`, icon: UserCheck },
    { label: 'Kontakt', path: `/${currentSlug}/kontakt`, icon: Mail },
  ];
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 w-full transition-all duration-500 ${isScrolled ? 'py-1.5 sm:py-2' : 'py-2.5 sm:py-4'}`}>
      <div className="max-w-7xl mx-auto px-2.5 sm:px-6 relative space-y-1.5 sm:space-y-2">
        ...
        {/* TOP BAR: City title, Switcher, Dynamic Phone & Socials */}
        <div className="hidden sm:flex items-center justify-between px-4 py-1.5 rounded-2xl bg-black/60 border border-white/10 text-[11px] font-bold text-slate-300 backdrop-blur-xl shadow-lg">
          {/* Top-bar Left: City Title & Selector Dropdown */}
          <div className="flex items-center gap-3 relative">
            <span className="text-orange-400 font-black uppercase tracking-wider flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5 text-orange-400 animate-pulse" />
              {currentLocation.topBarTitle || `${currentLocation.name.toUpperCase()} - ${currentLocation.mall.toUpperCase()}`}
            </span>
                  ...
                  >
                    ...
                    {LOCATIONS_DATA.map((loc) => (
                      ...
                      >
                        ...
                        {currentSlug === loc.id && <div className="w-2 h-2 rounded-full bg-orange-400 animate-ping" />}
                      </button>
                    ))}
                  </motion.div>
                )}
          ...
          {/* Top-bar Right: Location Phone & Socials */}
          <div className="flex items-center gap-4 text-xs">
            <a
              href={`tel:${currentLocation.phoneClean}`}
        ...
        <div className={`relative rounded-[20px] sm:rounded-[26px] border transition-all duration-500 flex justify-between items-center overflow-hidden ${
          ...
          >
            {navItems.map((item) => {
              const isActive = location.pathname === item.path || (item.path === `/${currentSlug}` && location.pathname === `/${currentSlug}`);
              return (
                <button
                  key={item.path}
                  onClick={() => {
          ...
          <div className="hidden sm:flex items-center gap-3 relative z-10">
            <motion.button
              ...
              whileTap={{ scale: 0.96 }}
              onClick={() => openBooking(currentSlug)}
              ...
              <div className="rounded-[22px] border border-white/15 bg-slate-950/95 backdrop-blur-2xl p-4 flex flex-col gap-2 shadow-[0_20px_50px_rgba(0,0,0,0.95)] text-left">
                <div className="flex justify-between items-center border-b border-white/10 pb-2.5 px-1">
                  <span className="text-[10px] font-black tracking-[0.2em] text-orange-400 uppercase flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5" /> Grawitacja {currentLocation.name}
                  </span>
                  <a
                    href={`tel:${currentLocation.phoneClean}`}
