...
export default function PoznanPage() {
  const poznan = LOCATIONS_DATA.find((l) => l.id === 'poznan') || LOCATIONS_DATA[2];
  const promotions = PROMOTIONS_DATA.filter((p) => !p.locationId || p.locationId === 'poznan');
  ...
  return (
    <div className="space-y-12 py-4 text-left">
      ...
      <section className="relative rounded-[32px] p-6 sm:p-10 md:p-12 border border-white/10 bg-gradient-to-b from-slate-900/90 via-slate-950/80 to-black/90 backdrop-blur-3xl shadow-[0_20px_60px_rgba(0,0,0,0.9)] overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-7 space-y-6">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 text-xs font-black tracking-widest uppercase">
              <Sparkles className="w-3.5 h-3.5" /> Centrum Poznań • {poznan.mall}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2">
              <div className="p-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
                ...
                <span className="text-lg font-black text-white">{poznan.uvLanes || poznan.lanes}</span>
              </div>
              <div className="p-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
                <span className="block text-[10px] text-slate-400 font-bold uppercase tracking-wider">Strefa Rozrywki</span>
                <span className="text-lg font-black text-white">{poznan.zones}</span>
              </div>
              <div className="p-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md col-span-2 sm:col-span-1">
                ...
                <span className="text-sm font-black text-purple-400">{poznan.phone}</span>
              </div>
            </div>

            <div className="flex flex-wrap gap-4 pt-4">
              <button className="px-8 py-4 rounded-2xl text-xs font-black tracking-widest uppercase text-white bg-gradient-to-r from-purple-500 via-pink-600 to-purple-500 bg-[length:200%_auto] hover:bg-right transition-all duration-500 shadow-[0_0_30px_rgba(168,85,247,0.5)] flex items-center gap-2 cursor-pointer">
            ...
            <div className="p-6 rounded-3xl bg-slate-900/60 border border-white/10 backdrop-blur-2xl space-y-4 shadow-xl">
              <div className="flex items-center gap-3 border-b border-white/10 pb-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-white">{poznan.address} ({poznan.mall})</p>
                </div>
              </div>

              <div className="space-y-3">
                <h4 className="text-xs font-black uppercase text-slate-400 tracking-wider flex items-center gap-2">
                  <Clock className="w-4 h-4 text-purple-400" /> Godziny otwarcia
                </h4>
                <div className="space-y-2 text-xs font-medium">
                  {poznan.hours.map((item, index) => (
                    <div
                      key={index}
      ...
      <LaneDivider label="10 UV-TORÓW • GLOW BOWLING POZNAŃ" badge="POZNAŃ" />
      ...
      <section className="space-y-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 border-b border-white/10 pb-4">
          <div>
            <span className="text-[10px] font-black tracking-[0.2em] text-purple-400 uppercase">Oferty Specjalne</span>
            <h2 className="text-2xl sm:text-4xl font-black uppercase tracking-tight text-white">Promocje Poznań</h2>
          </div>
        </div>
